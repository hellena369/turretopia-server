...
const {roomLoop} = require("./modules/setup/room");
const {speedcheckloop} = require("./modules/debug/speedLoop");
...
const maintainloop = () => {
    ...
    if (!global.arenaClosed && bots.length < Config.BOTS) {
        ...
        o.color.base = Config.RANDOM_COLORS ? Math.floor(Math.random() * 20) : team ? getTeamColor(team) : "darkGrey";
        if (team) o.team = team;
        bots.push(o);
        setTimeout(() => {
            // allow them to move
