...
exports.combineGuns = (...tanks) => {
    const combinedGuns = [];

    tanks.forEach(tank => {
        tank = ensureIsClass(tank);
        if (tank.GUNS) {
            combinedGuns.push(...tank.GUNS);
        }
    });
...
exports.makeAuto = (type, name = -1, options = {}) => {
    ...
    if (name == -1) {
        if (options.type === 'megaAutoTurret') {
          output.LABEL = 'Mega Auto-' + type.LABEL;
        } else if (options.type === 'tripleAutoTurret') {
...
exports.makeTurret = (type, options = {}) => {
    ...
    */
    ...
    return {
        ...
        FACING_TYPE: options.facingType ?? "toTarget",
        CONTROLLERS,
        GUNS,
...
exports.makeCrasher = (type, color) => {
    type = ensureIsClass(type);
    return {
    PARENT: type,
    COLOR: color,
    ...
    VALUE: type.VALUE * 5,
    BODY: {
        SPEED: 1 + 5 / Math.max(2, (type.SHAPE ?? 0) + type.SHAPE / 2),
