    ...
    o.on('dead', () => {
        ...
        if (team === mainTeam) {
            ...
            if (!teamcounts[team] && !gameWon) {
                gameWon = true;
                setTimeout(sockets.broadcast, 1500, "The raiders have won the game!"); //figure out how to guess a better text
                setTimeout(closeArena, 4500);
            }
...
let dominatorBlue = new Tile({ init: tile => makeDefenderDominator(tile, TEAM_BLUE, TEAM_BLUE, TEAM_ENEMIES) }),
    ...
    sirenBlue = new Tile({ init: tile => makeDefenderDominator(tile, TEAM_BLUE, TEAM_BLUE, TEAM_ENEMIES, 'siren') }),
